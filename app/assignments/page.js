  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-extrabold text-stone-900">काम सौंपें</h1>
        <button onClick={() => setShow(true)} className="bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors">+ काम सौंपें</button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md space-y-4 shadow-xl">
            ...
            <input placeholder="मजदूरी (₹)" value={returnForm.wages} onChange={e => setReturnForm({...returnForm, wages: e.target.value})}
              className="w-full border border-gray-200 rounded-lg px-4 py-2 text-sm outline-none focus:border-amber-400" />
            <div className="flex gap-3 pt-2">
              <button onClick={submitReturn} className="flex-1 bg-amber-500 hover:bg-amber-600 text-white font-semibold py-2 rounded-lg text-sm">सेव करें</button>
              <button onClick={() => setReturnShow(false)} className="flex-1 border border-gray-200 text-stone-600 font-semibold py-2 rounded-lg text-sm">रद्द करें</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={8} className="px-5 py-8 text-center text-stone-400 font-medium">कोई काम नहीं</td></tr>
            ) : rows.map(row => (
