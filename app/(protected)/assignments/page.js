...
export default function Assignments() {
  ...
  const waMsg = (row) => encodeURIComponent(`नमस्ते ${row.karigarName}, आपका ${row.lotNo} लॉट का काम (${row.stage}) वापस करने का समय हो गया है। कृपया जल्द संपर्क करें।`);
      ...
      {returnShow && selected && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md space-y-4 shadow-xl">
            ...
            <input placeholder="वापस वजन (कैरेट)" value={returnForm.returnedWeight} onChange={e => setReturnForm({...returnForm, returnedWeight: e.target.value, wages: calcWages(e.target.value)})}
              className="w-full border border-gray-200 rounded-lg px-4 py-3 text-base outline-none focus:border-amber-400" />
        ...
        {rows.length === 0 ? (
          ...
          <div key={row.id} className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
            ...
            <div className="flex gap-3 pt-1">
              ...
              {row.karigarPhone && row.status === "लंबित" && (
                <a href={`https://wa.me/91${row.karigarPhone}?text=${waMsg(row)}`} target="_blank" className="text-green-500 text-sm font-semibold">WhatsApp</a>
              )}
            ...
            {rows.length === 0 ? (
              ...
              <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-5 py-3 flex gap-2">
                  ...
                  {row.karigarPhone && row.status === "लंबित" && (
                    <a href={`https://wa.me/91${row.karigarPhone}?text=${waMsg(row)}`} target="_blank" className="text-green-500 hover:text-green-700 text-xs font-semibold">WhatsApp</a>
                  )}
