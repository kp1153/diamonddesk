...
export default function Assignments() {
  ...
  const calcWages = (weight) => {
    if (!selected || !weight) return "";
    const w = parseFloat(weight);
    if (isNaN(w)) return "";
    const stageRateMap = {
      आरी: selected.rateAari,
      घिसाई: selected.rateGhisai,
      पॉलिश: selected.ratePolish,
      जाँच: selected.rateJanch,
    };
    const rate = parseFloat(stageRateMap[selected.stage]) || 0;
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (karigarsData && Array.isArray(karigarsData))
  ...
  const load = () =>
    safeFetch("/api/assignments").then((data) => {
  ...
  const submitReturn = async () => {
    await fetch("/api/assignments", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: selected.id, ...returnForm }),
    });
    setReturnShow(false);
    setSelected(null);
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md space-y-4 shadow-xl">
            ...
            >
              ...
              {lots.map((l) => (
                <option key={l.id} value={l.id}>
                  {l.lotNo} ({l.roughWeight} कैरेट)
                </option>
              ))}
            </select>
            <input
              placeholder="वजन (कैरेट)"
              value={form.issuedWeight}
              onChange={(e) =>
                setForm({ ...form, issuedWeight: e.target.value })
              }
              className="w-full border border-gray-200 rounded-lg px-4 py-3 text-base outline-none focus:border-amber-400"
            />
            <select
            ...
            <div className="flex gap-3 pt-2">
              <button
                onClick={submit}
                className="flex-1 bg-amber-500 hover:bg-amber-600 text-white font-semibold py-3 rounded-lg text-base"
              >
                सेव करें
              </button>
              <button
                onClick={() => setShow(false)}
                className="flex-1 border border-gray-200 text-stone-600 font-semibold py-3 rounded-lg text-base"
              >
                रद्द करें
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md space-y-4 shadow-xl">
            <h2 className="text-lg font-bold text-stone-900">
              काम वापस लें — {selected.karigarName}
            </h2>
            <input
              placeholder="वापस वजन (कैरेट)"
              value={returnForm.returnedWeight}
              onChange={(e) =>
                setReturnForm({
                  ...returnForm,
                  returnedWeight: e.target.value,
                  wages: calcWages(e.target.value),
                })
              }
              className="w-full border border-gray-200 rounded-lg px-4 py-3 text-base outline-none focus:border-amber-400"
            />
            <input
              placeholder="मजदूरी (₹)"
              value={returnForm.wages}
              onChange={(e) =>
                setReturnForm({ ...returnForm, wages: e.target.value })
              }
              className="w-full border border-gray-200 rounded-lg px-4 py-3 text-base outline-none focus:border-amber-400"
            />
            <div className="flex gap-3 pt-2">
      ...
      <div className="md:hidden space-y-3">
        {rows.length === 0 ? (
          ...
          rows.map((row) => (
            ...
            >
              <div className="flex items-center justify-between">
                <p className="font-extrabold text-stone-800 text-lg">
                  {row.karigarName}
                </p>
                <span
                  className={`text-xs font-bold px-2 py-1 rounded-full ${row.status === "लंबित" ? "bg-orange-100 text-orange-600" : "bg-green-100 text-green-600"}`}
                >
                  {row.status}
                </span>
              </div>
              <p className="text-stone-500 text-sm">
                लॉट: {row.lotNo} | काम: {row.stage}
              </p>
              <p className="text-stone-500 text-sm">
                दिया: {row.issuedWeight} कैरेट | वापस:{" "}
                {row.returnedWeight ? `${row.returnedWeight} कैरेट` : "—"}
              </p>
              <p className="text-stone-500 text-sm">
                मजदूरी: {row.wages ? `₹${row.wages}` : "—"}
              </p>
          ...
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              {[
                ...
                <th
                  key={h}
                  className="px-5 py-3 text-left text-xs font-bold text-stone-500 uppercase tracking-widest"
                >
                  {h}
                </th>
              ))}
