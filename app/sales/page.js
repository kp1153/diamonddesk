...
export default function Sales() {
  ...
  const [form, setForm] = useState({ invoice_no: "", buyer: "", date: "", weight: "", amount: "", status: "Pending" });
  ...
  const submit = async () => {
    ...
    setShow(false);
    setForm({ invoice_no: "", buyer: "", date: "", weight: "", amount: "", status: "Pending" });
    load();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-extrabold text-stone-900 tracking-tight">Sales</h1>
        <button onClick={() => setShow(true)} className="bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors">+ Add Sale</button>
      </div>
      ...
      {show && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md space-y-4 shadow-xl">
            ...
            {[["invoice_no","Invoice No."],["buyer","Buyer"],["date","Date (YYYY-MM-DD)"],["weight","Weight (Ct)"],["amount","Amount (₹)"]].map(([k,l]) => (
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              {["Invoice No.","Buyer","Date","Weight","Amount","Status",""].map(h => (
