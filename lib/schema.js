...
export const users = sqliteTable("users", {
  ...
  googleId: text("google_id").unique(),                    // ← नया
  email: text("email").notNull().unique(),
  name: text("name"),
  image: text("image"),
  shopName: text("shop_name"),                             // ← नया
  whatsapp: text("whatsapp"),                              // ← नया
  status: text("status").notNull().default("trial"),
  active: integer("active").notNull().default(0),          // ← नया
...
export const karigars = sqliteTable("karigars", {
  ...
  createdAt: text("created_at").default(sql`(datetime('now'))`),
  rateAari: real("rate_aari").default(0),
  rateGhisai: real("rate_ghisai").default(0),
  ratePolish: real("rate_polish").default(0),
  rateJanch: real("rate_janch").default(0),
});
